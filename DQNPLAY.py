...
class Game(object):
    ...
    def step(self, action):
        ...
        if action == MOVE_LEFT:
            self.bar_pos_x = self.bar_pos_x - 4
...
input_image = tf.placeholder("float", [None, image_w, image_h, 4])  # 游戏像素
...
def convolutional_neural_network(input_image):
    weights = {'w_conv1': tf.Variable(tf.zeros([8, 8, 4, 32])),
               ...
               'w_fc4': tf.Variable(tf.zeros([128, 784])),
...
def train_neural_network(input_image):
    ...
    # 转换为灰度值
    image = cv2.cvtColor(cv2.resize(image, (image_h, image_w)), cv2.COLOR_BGR2GRAY)
    # 转换为二值
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            image = cv2.cvtColor(cv2.resize(image, (image_h, image_w)), cv2.COLOR_BGR2GRAY)
            ret, image = cv2.threshold(image, 1, 255, cv2.THRESH_BINARY)
            image = np.reshape(image, (image_w, image_h, 1))
